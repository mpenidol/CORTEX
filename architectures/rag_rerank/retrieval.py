"""Architecture: RAG + ReRank (arquitectura original de referencia).

Pipeline: query → vectorstore top-K → cross-encoder rerank → top-N al asset agent.

  K = numero de candidatos recuperados del vectorstore (ChromaDB) por objeto
  N = numero de candidatos que pasan al asset agent tras el cross-encoder reranker
      (N < K siempre; el reranker descarta K-N candidatos)
